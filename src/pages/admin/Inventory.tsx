  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-1 overflow-x-auto pb-1 border-b">
        {(['bikes', 'batteries', 'locations', 'pricing'] as const).map((tab) => {
...
function BikesTab() {
  ...
  const onSubmit = async (data: z.infer<typeof bikeSchema>) => {
    ...
    try {
      ...
      if (modal === 'new') {
        ...
        toast.success('Велосипед добавлен')
      } else if (modal) {
        await updateBike(modal.id, payload)
        toast.success('Велосипед обновлён')
      }
      setModal(null)
    } catch (err) {
  ...
  const openEdit = (bike: Bike) => {
    form.reset({
      ...
      type_id: bike.type_id ?? undefined,
      location_id: bike.location_id ?? undefined,
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button size="sm" onClick={openNew}>+ Добавить</Button>
      </div>
      ...
      <div className="hidden md:block overflow-x-auto rounded-2xl border bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-xs text-gray-400 uppercase tracking-wide text-left">
              <th className="px-4 py-3">Номер / VIN</th>
              <th className="px-4 py-3">Название</th>
              <th className="px-4 py-3">Статус</th>
              <th className="px-4 py-3">Куплен</th>
            ...
            {filtered.map((b) => (
              <tr key={b.id} className="border-b last:border-0 hover:bg-gray-50">
                <td className="px-4 py-3">
                  <p className="font-mono text-xs text-gray-700">{b.number}</p>
                  <p className="font-mono text-xs text-gray-400">{b.vin}</p>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(b)} className="text-brand-600 hover:underline text-xs">Изм.</button>
                    <button onClick={() => handleDelete(b.id)} className="text-red-500 hover:underline text-xs">Удал.</button>
                  </div>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-gray-400">Нет велосипедов</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="flex flex-col gap-3 md:hidden">
        {filtered.map((b) => (
          <div key={b.id} className="rounded-2xl bg-white border shadow-sm p-4">
            <div className="flex items-start justify-between mb-2">
              <div>
                <p className="font-medium text-gray-900">{b.name}</p>
                <p className="text-xs font-mono text-gray-400">{b.number} · {b.vin}</p>
              </div>
              <StatusBadge status={b.status} />
            </div>
            <div className="flex gap-2 mt-2">
              <button onClick={() => openEdit(b)} className="text-xs text-brand-600 hover:underline">Изменить</button>
              <button onClick={() => handleDelete(b.id)} className="text-xs text-red-500 hover:underline">Удалить</button>
            </div>
          </div>
        ))}
        {filtered.length === 0 && <p className="text-center py-8 text-gray-400">Нет велосипедов</p>}
      </div>

      {/* Modal */}
      <Modal isOpen={modal !== null} onClose={() => setModal(null)} title={modal === 'new' ? 'Добавить велосипед' : 'Редактировать'}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select className="rounded-lg border border-gray-300 px-3 py-2 text-sm" {...form.register('status')}>
...
function BatteriesTab() {
  ...
  const onSubmit = async (data: z.infer<typeof batterySchema>) => {
    ...
    try {
      ...
      if (modal === 'new') {
        ...
        toast.success('АКБ добавлен')
      } else if (modal) {
        await updateBattery(modal.id, payload)
        toast.success('АКБ обновлён')
      }
      setModal(null)
    } catch (err) {
      toast.error(extractErrorMessage(err))
  ...
  return (
    <>
      ...
      {/* Mobile cards + desktop table shared pattern */}
      <div className="flex flex-col gap-3">
        {batteries.map((b) => (
          <div key={b.id} className="rounded-2xl bg-white border shadow-sm p-4 flex items-start justify-between">
            <div>
              <p className="font-medium text-gray-900">{b.name}</p>
              <p className="text-xs font-mono text-gray-400">{b.number}</p>
              {b.voltage && <p className="text-xs text-gray-500 mt-0.5">{b.voltage}V · {b.capacity}Ah</p>}
            </div>
            <div className="flex flex-col items-end gap-2">
              <StatusBadge status={b.status} />
              <div className="flex gap-2">
                <button onClick={() => { form.reset({ ...b, description: b.description ?? '', location_id: b.location_id ?? undefined, purchase_date: b.purchase_date ?? '', voltage: b.voltage ?? undefined, capacity: b.capacity ?? undefined }); setModal(b) }} className="text-xs text-brand-600 hover:underline">Изм.</button>
                <button onClick={() => handleDelete(b.id)} className="text-xs text-red-500 hover:underline">Удал.</button>
              </div>
            </div>
          </div>
        ))}
        {batteries.length === 0 && <p className="text-center py-8 text-gray-400">Нет АКБ</p>}
      </div>
      ...
      <Modal isOpen={modal !== null} onClose={() => setModal(null)} title={modal === 'new' ? 'Добавить АКБ' : 'Редактировать АКБ'}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-gray-700">Статус</label>
            <select className="rounded-lg border border-gray-300 px-3 py-2 text-sm" {...form.register('status')}>
              {(['free', 'rented', 'repair', 'decommissioned'] as const).map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Input label="Дата покупки" type="date" {...form.register('purchase_date')} />
...
function LocationsTab() {
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3">
        {locations.map((l) => (
          <div key={l.id} className="rounded-2xl bg-white border shadow-sm p-4 flex justify-between items-start">
            <div>
              <p className="font-medium text-gray-900">{l.name}</p>
              <p className="text-sm text-gray-500">{l.address}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => { form.reset(l); setModal(l) }} className="text-xs text-brand-600 hover:underline">Изм.</button>
              <button onClick={() => handleDelete(l.id)} className="text-xs text-red-500 hover:underline">Удал.</button>
            </div>
          </div>
        ))}
        {locations.length === 0 && <p className="text-center py-8 text-gray-400">Нет локаций</p>}
      </div>
      <Modal isOpen={modal !== null} onClose={() => setModal(null)} title={modal === 'new' ? 'Новая локация' : 'Редактировать'}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3">
          <Input label="Название" error={form.formState.errors.name?.message} {...form.register('name')} />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-xs text-gray-400 uppercase tracking-wide text-left">
              ...
              <th className="px-4 py-3">Недели</th>
              <th className="px-4 py-3">Цена/нед.</th>
              <th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody>
            {pricing.map((p) => (
              <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50">
                <td className="px-4 py-3">
                  <p className="font-medium">{p.name_type}</p>
                  <p className="text-xs text-gray-400">ID: {p.type_id}</p>
                </td>
                <td className="px-4 py-3 text-gray-600">{p.min_weeks_count}–{p.max_weeks_count} нед.</td>
                <td className="px-4 py-3 font-medium">{p.amount_weeks.toLocaleString('ru-RU')} ₽</td>
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => { form.reset(p); setModal(p) }} className="text-xs text-brand-600 hover:underline">Изм.</button>
                    <button onClick={() => handleDelete(p.id)} className="text-xs text-red-500 hover:underline">Удал.</button>
                  </div>
                </td>
              </tr>
            ))}
            {pricing.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-400">Нет тарифов</td></tr>}
          </tbody>
        </table>
      </div>
